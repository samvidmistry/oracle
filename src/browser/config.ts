...
export function resolveBrowserConfig(
	...
	const provider = config?.provider ?? DEFAULT_BROWSER_CONFIG.provider ?? "chatgpt";
	const isM365 = provider === "m365-copilot";
	const defaultUrl = isM365 ? "https://copilot.cloud.microsoft/" : DEFAULT_BROWSER_CONFIG.url;
	const rawUrl = config?.chatgptUrl ?? config?.url ?? defaultUrl;
	const normalizedUrl = normalizeChatgptUrl(rawUrl ?? defaultUrl, defaultUrl);
	const desiredModel = isM365
		? (config?.desiredModel ?? null)
		: (config?.desiredModel ?? DEFAULT_BROWSER_CONFIG.desiredModel ?? DEFAULT_MODEL_TARGET);
	const modelStrategy = isM365
	...
	const manualLogin = isM365
		? true
		: (config?.manualLogin ?? (isWindows ? true : DEFAULT_BROWSER_CONFIG.manualLogin));
	const cookieSyncDefault = isM365 ? false : isWindows ? false : DEFAULT_BROWSER_CONFIG.cookieSync;
	...
	return {
		...
		provider,
		url: normalizedUrl,
		chatgptUrl: isM365 ? null : normalizedUrl,
