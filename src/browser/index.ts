...
import {
	captureAssistantMarkdown as captureM365AssistantMarkdown,
	ensureLoggedIn as ensureM365LoggedIn,
	ensurePromptReady as ensureM365PromptReady,
	M365_COPILOT_URL,
	navigateToCopilot,
	readAssistantSnapshot as readM365AssistantSnapshot,
	submitPrompt as submitM365Prompt,
	waitForAssistantResponse as waitForM365AssistantResponse,
} from "./m365/index.js";
import {
...
export async function runBrowserMode(options: BrowserRunOptions): Promise<BrowserRunResult> {
	...
	const isM365 = config.provider === "m365-copilot";
	if (isM365 && attachments.length > 0) {
		throw new Error(
			"M365 Copilot provider does not support attachments yet. Remove --file inputs and retry.",
		);
	}
	...
	const manualProfileDir = config.manualLoginProfileDir
		...
		: path.join(os.homedir(), ".oracle", isM365 ? "m365-browser-profile" : "browser-profile");
	...
	if (manualLogin) {
		...
		const providerLabel = isM365 ? "M365 Copilot" : "ChatGPT";
		logger(
			`Manual login mode enabled for ${providerLabel}; reusing persistent profile at ${userDataDir}`,
		);
	} else {
	...
	try {
		...
		const baseUrl = isM365 ? M365_COPILOT_URL : CHATGPT_URL;
		if (isM365) {
			await raceWithDisconnect(navigateToCopilot(Page, Runtime, logger, baseUrl));
			await raceWithDisconnect(ensureM365LoggedIn(Runtime, logger));
			await raceWithDisconnect(ensureM365PromptReady(Runtime, config.inputTimeoutMs, logger));
		} else {
		...
		if (config.desiredModel && modelStrategy !== "ignore" && !isM365) {
			await withRetries(
			...
			logger("Model picker: skipped for M365 Copilot");
		}
		...
		const submitOnce = async (prompt: string, submissionAttachments: BrowserAttachment[]) => {
			const baselineSnapshot = await (isM365
				? readM365AssistantSnapshot(Runtime).catch(() => null)
			...
			if (isM365) {
				await submitM365Prompt(Runtime, Input, prompt, logger);
				baselineTurns = baselineTurns ?? 0;
		...
		try {
			...
			if (options.fallbackSubmission && isPromptTooLarge && !isM365) {
		...
		let answer = isM365
			? await waitForM365AssistantResponse(
					Runtime,
					config.timeoutMs,
					logger,
		...
		const copiedMarkdown = await withRetries(
			async () => {
				const attempt = await (isM365
					? captureM365AssistantMarkdown(Runtime, answer.meta, logger)
					: captureAssistantMarkdown(Runtime, answer.meta, logger));
				if (!attempt) {
		...
		if (isM365 && !copiedMarkdown) {
			logger("M365 Copilot: markdown copy not available; falling back to plain text.");
		}
		...
		const finalSnapshot = await (isM365
			? readM365AssistantSnapshot(Runtime, baselineTurns ?? undefined).catch(() => null)
	...
	try {
		...
		if (baselineNormalized) {
			...
			if (isBaseline) {
				...
				const refreshed = await waitForFreshAssistantResponse(baselineNormalized, 15_000);
				if (refreshed) {
					answerText = refreshed.text;
					answerHtml = refreshed.html ?? "";
				}
			}
		}
		if (!answerText) {
